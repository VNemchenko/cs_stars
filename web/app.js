(function() {
  ...
  qs('payBtn').addEventListener('click', async ()=> {
    ...
    if (!ok) { alert('Error: ' + error); qs('payBtn').disabled = false; return; }
    tg.openInvoice(link, (status)=>{
      if (status === 'paid') tg.showPopup({ title: 'Payment received', message: 'Provisioning your server. Bot will DM you shortly.' });
      else if (status === 'cancelled') qs('payBtn').disabled = false;
    });
  });
})();